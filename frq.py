...
for split in splits:
    dev_sentences = open('data/device/train/sentence.txt', 'r', encoding='utf-8').readlines()
    res_sentences = open('data/restaurant/train/sentence.txt', 'r', encoding='utf-8').readlines()
    lap_sentences = open('data/laptop/train/sentence.txt', 'r', encoding='utf-8').readlines()
    ser_sentences = open('data/service/train/sentence.txt', 'r', encoding='utf-8').readlines()
    ...
    total_cnt, word_cnt, ser_cnt, ser_word_cnt = count_word_and_class(ser_sentences, total_cnt, word_cnt, ser_cnt, ser_word_cnt)

    pmi_dict = {}
    frq_dict = {}
    for word in word_cnt:
        pmi_dict[word] = [0., 0., 0., 0.]
        frq_dict[word] = [0, 0, 0, 0]
    ...
    for word in frq_dict:
        ...
        if word in ser_word_cnt:
            frq_dict[word][3] = ser_word_cnt[word]
    ...
    # pmi_dict = calculate_pmi(pmi_dict, total_cnt, word_cnt, lap_cnt, lap_word_cnt, 'ser')
    ...
    with open('./data/frq_dict.txt', 'w', encoding='utf-8') as f:
