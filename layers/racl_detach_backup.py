...
class RACL_Layer(nn.Module):
    ...
    def forward(self, inputs, mask, position):
        ...
        for hop in range(self.opt.hop_num):
            ...
            # query_now = (query[-1] + torch.matmul(word_att_context, context_conv) + aspect_conv) # query + value + aspect
            query_now = (query[-1] + torch.matmul(word_att_context, context_conv) + aspect_conv.detach()) # query + value + aspect
