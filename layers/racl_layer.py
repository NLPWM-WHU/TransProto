...
class RACL_Layer(nn.Module):
    def __init__(self, input_dim, opt):
        ...
        self.aspect_convs = nn.ModuleList([nn.Sequential(nn.Conv1d(768*2, 768*2, 3, padding=1), nn.ReLU())
                                           for i in range(self.opt.hop_num)])

        # self.aspect_convs = nn.ModuleList([nn.Sequential(nn.Conv1d(768*2, 768, 3, padding=1), nn.ReLU())] + \
        #                                     [nn.Sequential(nn.Conv1d(768, 768, 3, padding=1), nn.ReLU()) \
        #                                    for i in range(self.opt.hop_num - 1)])
